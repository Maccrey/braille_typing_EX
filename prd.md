...
본 프로젝트는 사용자가 점자 타자 능력을 향상시킬 수 있도록 돕는 웹 애플리케이션을 구축하는 것을 목표로 한다. 사용자는 회원가입 및 로그인을 통해 개인화된 학습 환경을 제공받으며, 다양한 데이터 항목(카테고리)을 선택하여 점자 입력을 연습할 수 있다. 관리자는 Excel 파일을 업로드하여 손쉽게 학습 데이터를 추가하거나 관리할 수 있다. 사용자의 학습 진행 상황은 연습 시간과 출석 달력을 통해 시각적으로 제공되어 자기 주도적 학습을 유도한다.
...
- **[AUTH-1] 로그인 페이지:**
  - 사용자는 이름과 비밀번호를 입력하여 로그인한다.
  - 로그인 성공 시 메인 메뉴로 이동한다.
  - "회원가입" 및 "회원 탈퇴" 버튼을 포함한다.
- **[AUTH-2] 회원가입:**
  - 사용자는 이름과 비밀번호를 설정하여 계정을 생성한다.
  - 이름은 중복될 수 없다.
- **[AUTH-3] 회원 탈퇴:**
  - 로그인한 사용자는 계정을 영구적으로 삭제할 수 있다.
  - 탈퇴 시 관련 데이터(학습 기록 등) 처리 방안을 명시해야 한다.
